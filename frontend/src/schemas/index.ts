...
const CITIES = [
    ...
    'Талдықорған', 'Екібастұз', 'Теміртау', 'Жанаозен', 'Рудный', 'Мақат'
] as const;

const RAW_TO_CITIES = [
    'Актау', 'Астана', 'Атырау', 'Актобе', 'Уральск', 'Куль сары', 'Жанаозен',
    'Бейнеу', 'Сексеул', 'Кандагаш', 'Казалы', 'Шалкар', 'Арал', 'Шетпе', 'Алга'
] as const;

type CityName = typeof CITIES[number] | typeof RAW_TO_CITIES[number];

const TO_CITIES = [...new Set([...CITIES, ...RAW_TO_CITIES])] as [CityName, ...CityName[]];
